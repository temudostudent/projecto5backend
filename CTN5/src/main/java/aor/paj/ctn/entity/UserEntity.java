...
@NamedQuery(name = "User.countUsersOverTime", query = "SELECT YEAR(u.confirmDate), WEEK(u.confirmDate), COUNT(u) FROM UserEntity u WHERE u.visible = true GROUP BY YEAR(u.confirmDate), WEEK(u.confirmDate) ORDER BY u.confirmDate ASC")
@NamedQuery(name = "User.countUsersByDay", query = "SELECT (u.confirmDate), COUNT(u) FROM UserEntity u WHERE u.visible = true GROUP BY (u.confirmDate) ORDER BY u.confirmDate ASC")
